...
{domains}
Boundary rules: choose tool_use only when the request explicitly requires invoking an available external tool/function/API to perform an action. Discussing or debugging APIs, databases, distributed services, or software remains code. Code manipulation/debugging is code even when it contains math; a mathematical calculation or proof is math even though it requires reasoning; logic is for non-mathematical reasoning/puzzles. If the requested deliverable is rewriting, summarizing, translating, formatting, constrained composition, or creative text, choose instruction_following—not knowledge. Knowledge is for answering factual/substantive questions, not transforming supplied content.
Examples: fix Python syntax→code; debug a distributed-system race→code; prove infinitely many primes→math; solve a seating deduction puzzle→logic; answer a factual science question→knowledge; rewrite text under style constraints→instruction_following; use an available calendar tool to book a meeting→tool_use; identify a speaker's emotion→affective.
