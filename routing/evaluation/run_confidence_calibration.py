...
import pandas as pd
from routing.evaluation.confidence_calibration import calibration_report,save_calibration_artifact


def main():
    p=argparse.ArgumentParser();p.add_argument("predictions");p.add_argument("--output",required=True);p.add_argument("--artifact");args=p.parse_args()
    ...
    if args.artifact:save_calibration_artifact(frame,Path(args.artifact))
