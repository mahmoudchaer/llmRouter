...
def tiers_to_cumulative(tiers_zero_based,num_tiers=4):
    thresholds=torch.arange(1,num_tiers,device=tiers_zero_based.device)
...
class MonotonicOrdinalHead(nn.Module):
    ...
    def __init__(self,input_dim,num_tiers=4):
        super().__init__();self.num_tiers=num_tiers;self.score=nn.Linear(input_dim,1);self.first_cutpoint=nn.Parameter(torch.tensor(-1.0));self.cutpoint_deltas=nn.Parameter(torch.full((max(0,num_tiers-2),),0.5))
    def cutpoints(self):
        deltas=F.softplus(self.cutpoint_deltas)+1e-4
        return torch.cat([self.first_cutpoint.view(1),self.first_cutpoint+deltas.cumsum(0)])
...
def cumulative_ordinal_loss(logits,tiers_zero_based,under_route_weight=1.0):
    targets=tiers_to_cumulative(tiers_zero_based,logits.shape[1]+1).to(logits.dtype)
...
def cumulative_probabilities(logits):
    ...
    return torch.cat([1-survival[:,:1],middle,survival[:,-1:]],dim=1)
...
def selection_score(macro_f1,mae,under_routing,severe_under_routing,max_error=3):
    return .35*(mae/max_error)+.35*severe_under_routing+.20*under_routing+.10*(1-macro_f1)
