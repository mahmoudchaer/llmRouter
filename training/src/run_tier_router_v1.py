...
def prepare(cfg,data_path,split_path,cache_path):
    ...
    ids=[tok.encode(str(x),add_special_tokens=False) for x in df.prompt];chunks=[chunk_token_ids(x,2048,128,tok.eos_token_id) for x in ids]
...
def metrics(y,pred,prob,cost_matrix):
    y=np.asarray(y);pred=np.asarray(pred);delta=pred-y;onehot=np.eye(4)[y-1]
    ...
    return {"exact_accuracy":float(accuracy_score(y,pred)),"macro_f1":f1,"mae":mae,"within_one_tier":float((np.abs(delta)<=1).mean()),"under_routing":under,"over_routing":float((delta>0).mean()),"severe_under_routing":severe,"T4_to_T1":int(((y==4)&(pred==1)).sum()),"T4_to_T2":int(((y==4)&(pred==2)).sum()),"T3_to_T1":int(((y==3)&(pred==1)).sum()),"routing_cost":float(costs.mean()),"selection_score":selection_score(f1,mae,under,severe),"log_loss":float(log_loss(y,prob,labels=[1,2,3,4])),"brier":float(((prob-onehot)**2).sum(1).mean()),"confusion_matrix":confusion_matrix(y,pred,labels=[1,2,3,4]).tolist()}
...
def main():
    p=argparse.ArgumentParser();p.add_argument("--name",required=True);p.add_argument("--loss",choices=["ce","ordinal"],required=True);p.add_argument("--under-weight",type=float,default=1);p.add_argument("--no-struct",action="store_true");p.add_argument("--eval-test",action="store_true");p.add_argument("--checkpoint");p.add_argument("--config",default="configs/tier_router_v1.yaml");p.add_argument("--output-root",default="output/tier_router_v1");p.add_argument("--max-train",type=int);p.add_argument("--max-validation",type=int);p.add_argument("--epochs",type=int);args=p.parse_args()
    cfg=yaml.safe_load(open(args.config));seed_all(17);budget=cfg["training"]["single_chunk_batch_token_budget"];records,scaler,pad=prepare(cfg,"data/final_labeled_dataset.parquet","splits/grouped_split_17.json","cache/tier_router_v1_seed17.pkl");parts={x:[r for r in records if r["partition"]==x] for x in ["train","validation","test"]};out=Path(args.output_root)/args.name;out.mkdir(parents=True,exist_ok=True);structural=not args.no_struct
    model=build_tier_router(cfg,"cuda",args.loss,structural);cost=cfg["evaluation_cost_matrix"]["matrix"]
    run_cfg={"name":args.name,"loss":args.loss,"under_weight":args.under_weight,"structural":structural,"seed":17,"git_commit":subprocess.check_output(["git","rev-parse","HEAD"],text=True).strip(),"model":cfg["model"],"training":cfg["training"],"selection":cfg["selection"]}
    if args.eval_test:
    ...
    for epoch in range(1,epochs+1):
        ...
        for kind,item in work:
            ...
            if kind=="single":loss,tok=train_batch(model,item,pad,structural,args.under_weight);tokens+=tok
                ...
                feature=torch.tensor(item["structural"],device="cuda",dtype=next(model.parameters()).dtype) if structural else None;loss=tier_streaming_backward(model,item["chunks"],feature,torch.tensor(item["tier"],device="cuda"),pad,2048,args.under_weight,170000+epoch*1000+global_step);tokens+=2*sum(map(len,item["chunks"]))
